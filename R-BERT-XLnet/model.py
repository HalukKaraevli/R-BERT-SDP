...
class XLNetForSequenceClassification(XLNetPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.sdp_dropout = nn.Dropout(0.1) # dropout on SDP CLS
    ...
    def forward(self,
        ...
        ##Addition
        sdp_output = self.transformer(sdp_ids,
                                      position_ids=position_ids,
                                      attention_mask=sdp_mask,
                                      head_mask=None)
        sdp_output = self.sequence_summary(sdp_output[0])

        # for details, see https://huggingface.co/transformers/model_doc/bert.html#bertmodel
        pooled_output = self.sequence_summary(transformer_outputs[0])  # sequence of hidden-states at the output of the last layer of the model
        sequence_output = transformer_outputs[0]  # last layer hidden-state of the first token of the sequence (classification token) further processed by a Linear layer and a Tanh activation function.
        ...
        ##Addition
        sdp = self.sdp_dropout(sdp_output)
        
        pooled_output = torch.cat([context, sdp, e1_h, e2_h], dim=-1)
