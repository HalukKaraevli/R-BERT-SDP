...
from transformers import (WEIGHTS_NAME, XLNetConfig, XLNetTokenizer, XLNetModel, BertConfig)
...
from model import XLNetForSequenceClassification
...
def load_and_cache_examples(config, task, tokenizer, evaluate=False, test=True):
    ...
    all_sdp_ids = torch.tensor(
        [f.sdp_ids for f in features], dtype=torch.long)
    all_sdp_mask =torch.tensor(
        [f.sdp_mask for f in features], dtype=torch.long)
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_sdp_ids, all_sdp_mask,
...
def main():
    parser = ArgumentParser(
        description="XLNet for relation extraction (classification)")
    ...
    xlnetconfig = XLNetConfig.from_pretrained(
    ...
    print(tokenizer.vocab_size, 'Tokenizer vocab size')
    model = XLNetForSequenceClassification.from_pretrained(
        config.pretrained_model_name, config=xlnetconfig)
    ...
    model.resize_token_embeddings(len(tokenizer)) ## The solution?

    # Training
    if config.train:
        train_dataset = load_and_cache_examples(
            config, config.task_name, tokenizer, evaluate=False)
        print(tokenizer.convert_ids_to_tokens([30522, 30523, 30524, 30525]))
    ...
    if config.train and (config.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = XLNetForSequenceClassification.from_pretrained(config.output_dir)
        tokenizer = XLNetTokenizer.from_pretrained(
    ...
    pickle.dump(results, open('rb_results.pkl','wb'))
    return results
