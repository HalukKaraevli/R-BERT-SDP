...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, sdp_text_a, text_b=None, label=None):
...
def convert_examples_to_features(examples, label_list, max_seq_len,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        sdp_tokens_a = tokenizer.tokenize(example.sdp_text_a)
        tokens_b = None
        ...
        sdp_tokens = sdp_tokens_a + [sep_token]
        ...
        sdp_tokens = [cls_token] + sdp_tokens
        segment_ids = [cls_token_segment_id] + segment_ids

        input_ids = tokenizer.convert_tokens_to_ids(tokens)
        sdp_ids = tokenizer.convert_tokens_to_ids(sdp_tokens)
        # entity mask
        ...
        sdp_mask = [1 if mask_padding_with_zero else 0] * len(sdp_ids)

        # Zero-pad up to the sequence length.
        padding_length = max_seq_len - len(input_ids)
        sdp_padding_length = max_seq_len - len(sdp_ids)
        input_ids = input_ids + ([pad_token] * padding_length)
        input_mask = input_mask + ([0 if mask_padding_with_zero else 1] * padding_length)
        sdp_ids = sdp_ids + ([pad_token] * sdp_padding_length)
        sdp_mask = sdp_mask + ([0 if mask_padding_with_zero else 1] * sdp_padding_length)
        ...
        if ex_index < 5:
            ...
            logger.info("sdp_ids: %s" %
                        " ".join([str(x) for x in sdp_ids]))
            logger.info("sdp_mask: %s" %
                        " ".join([str(x) for x in sdp_mask]))
            if use_entity_indicator:
